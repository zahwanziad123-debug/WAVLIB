...
export default function UploadPage() {
  ...
  async function sendLogin() {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email: ADMIN_EMAIL,
      options: { emailRedirectTo: `${window.location.origin}${BASE_PATH}/upload/` },
    });
    if (error) setError(error.message);
    else setMessage(`A secure admin sign-in link was sent to ${ADMIN_EMAIL}.`);
    setBusy(false);
  }
