...
async function ensureTag(name: string) {
  ...
  // Another upload may have created the same tag between the select and insert.
...
export async function createPackWithFiles(input: PackUploadInput, onProgress?: (completed: number, total: number) => void) {
  ...
  try {
    ...
    for (const file of samples) {
      ...
      if (tagRows.length) {
        const { error: linkError } = await supabase.from('sample_tags').insert(
          tagRows.map((tag) => ({ sample_id: sample.id, tag_id: tag.id })),
        );
