...
import enhanced from './[slug]/browser-enhanced.module.css';
...
function fallbackWaveform(index: number) { return Array.from({ length: 72 }, (_, i) => 5 + ((i * 17 + index * 13) % 25)); }
...
function Waveform({ sample, index }: { sample: LiveSample; index: number }) {
  ...
  return <div className={styles.waveform} aria-label="Exact audio waveform">{bars.map((height, i) => <i key={i} style={{ height: Math.max(4, Math.min(34, Number(height) || 4)) }} />)}</div>;
}
...
export default function UploadedPackDetail({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch (err) { if (!cancelled) setError(err instanceof Error ? err.message : 'Could not load this pack.'); }
  ...
  const hasFilters = type !== 'All' || tag !== 'All' || key !== 'All' || bpm !== 'All' || duration !== 'All' || !!query;
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.packHeader}>
      <div className={`${styles.packCover} ${enhanced.packCoverClean}`}>{pack.artworkUrl ? <img src={pack.artworkUrl} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 2 }} /> : null}</div>
    ...
    <section className={styles.browser}>
      <div className={styles.searchRow}>
        <label className={styles.localSearch}><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search this pack…"/><span>{filtered.length} / {samples.length} sounds</span></label>
        <div className={`${styles.filters} ${enhanced.filterBar}`}>
          ...
          {hasFilters && <button className={enhanced.clearFilter} onClick={clearFilters}>Clear all ×</button>}
        </div>
      </div>

      <div className={enhanced.typeTabs}>
        <button className={type === 'All' ? enhanced.tabActive : ''} onClick={() => setType('All')}>All <span>{typeCounts.All}</span></button>
        <button className={type === 'One Shot' ? enhanced.tabActive : ''} onClick={() => setType('One Shot')}>One Shots <span>{typeCounts['One Shot']}</span></button>
        <button className={type === 'Loop' ? enhanced.tabActive : ''} onClick={() => setType('Loop')}>Loops <span>{typeCounts.Loop}</span></button>
      </div>
      <div className={styles.tagStrip}>{allTags.slice(0, 16).map((value) => <button key={value} className={tag === value ? styles.tagSelected : ''} onClick={() => setTag(tag === value ? 'All' : value)}>{value}</button>)}</div>

      <div className={`${styles.sampleList} ${enhanced.enhancedList}`}>
        <div className={enhanced.sampleHeader}><span></span><span>NAME</span><span>WAVEFORM</span><span>KEY</span><span>BPM</span><span>TYPE</span><span>TIME</span><span>♥</span><span>↓</span></div>
        {filtered.map((sample, index) => <div className={`${styles.sample} ${enhanced.enhancedSample}`} key={sample.id}>
